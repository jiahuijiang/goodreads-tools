...
def predict_rating(all_friend_compare_results):
    all_friends = [compare_result.get("friend_id") for compare_result in all_friend_compare_results]
    all_book_urls = get_all_rated_books_urls(all_friend_compare_results)
    ...
    for friend_compare_result in all_friend_compare_results:
        ...
        for book in friend_compare_result.get("books_in_common"):
            if type(book.get("my_rating")) is int:
                score_df.at[book.get("partial_url"), my_id_placeholder] = book.get("my_rating")
            if type(book.get("their_rating")) is int:
    ...
    for book_index, book_title in list(enumerate(score_df.index)):
        ...
        if score_df.iloc[book_index, 0] == 0:
            ...
            if denominator != 0:
                predicted_ratings[book_title] = total_weighted_score / denominator
